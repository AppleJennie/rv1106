...
typedef struct {
    ...
    /* 连续失败后退避冷却，避免单次慢帧/偶发错误永久关闭 106 点。 */
...
{
    if (g_lm106.mock || !g_lm106.input_mem) {
        return NULL;
    }
    return (const uint8_t *)g_lm106.input_mem->virt_addr;
}
...
static bool run_inference(landmark_106_ctx_t *ctx)
{
    ...
    if (ret != RKNN_SUCC || cost_ms > 1000ULL) {
        ctx->consecutive_fail++;
        if (ctx->consecutive_fail >= 5) {
            ctx->consecutive_fail = 0;
            ctx->cooldown_until_us = get_mono_time_us() + 5000000ULL; /* 5s 后自动重试 */
            if (!ctx->cooldown_logged) {
                ctx->cooldown_logged = true;
                log_warn("landmark_106 连续 RKNN 失败/超时，进入 5s 降级冷却 (ret=%d, cost=%llums)",
                         ret, (unsigned long long)cost_ms);
            }
        } else {
            log_warn("landmark_106 RKNN 单帧失败/超时 ret=%d cost=%llums fail=%d",
                     ret, (unsigned long long)cost_ms, ctx->consecutive_fail);
        }
        return false;
    }

    ctx->consecutive_fail = 0;
    ctx->cooldown_logged = false;
    return true;
}
                                   ...
                                   dms_face_landmark_106_result_t *result)
{
    ...
    bool ok = dms_face_landmark_106_process_image(&img, face, result);
    dms_free_image(&img);
    return ok;
}
                                         ...
                                         dms_face_landmark_106_result_t *result)
{
    ...
    uint64_t now_us = get_mono_time_us();
    if (g_lm106.cooldown_until_us != 0 && now_us < g_lm106.cooldown_until_us) {
        mock_result(result);
        return true;
    }
    if (g_lm106.cooldown_until_us != 0 && now_us >= g_lm106.cooldown_until_us) {
        g_lm106.cooldown_until_us = 0;
        g_lm106.cooldown_logged = false;
        log_info("landmark_106 冷却结束，恢复真实推理");
    }
