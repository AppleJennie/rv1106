 ...
 */
#ifndef DMS_ENABLE_LANDMARK_106
#define DMS_ENABLE_LANDMARK_106    0
#endif

/*
 * DMS_DEBUG_LANDMARK_INDEX：106 点索引调试。
 *  1 = 在 debug 流给每个 landmark 画编号，仅用于确认 2d106det 语义索引
 *  0 = 正常显示 106 点，不显示编号
 */
#ifndef DMS_DEBUG_LANDMARK_INDEX
#define DMS_DEBUG_LANDMARK_INDEX    0
#endif

/*
 * DMS_HW_PREPROCESS：DMS Hardware Video Pipeline V2-A 总开关。
 *  1 = AI 主路径用 RGA 硬件预处理（VI NV12 DMABUF 直接转 640x640 BGR +
 *      半幅 RGB 源图），不再走软件 JPEG 解码 + CPU resize。
 *  0 = 完整走现有软件路径（stb JPEG decode + CPU preprocess），用于 A/B 回归。
 * 可被 CMake -DDMS_HW_PREPROCESS=OFF 覆盖（参考 DMS_ENABLE_LANDMARK_106）。
 */
#ifndef DMS_HW_PREPROCESS
#define DMS_HW_PREPROCESS    1
#endif

/* ==================== 疲劳驾驶特征参数（第一版，全部可调） ==================== */
#define DMS_EAR_CLOSE_RATIO        0.72f   /* 闭眼进入阈值 = ear_baseline * ratio */
#define DMS_EAR_RECOVER_RATIO      0.85f   /* 睁眼恢复阈值 = ear_baseline * ratio */
#define DMS_MAR_YAWN_MIN           0.45f   /* MAR 绝对下限，防止基线过低误报 */
#define DMS_MAR_YAWN_RATIO         1.60f   /* 哈欠阈值 = max(MIN, mar_baseline * ratio) */
#define DMS_MAR_RECOVER_RATIO      1.20f
#define DMS_HEAD_ENTER_DELTA       0.18f   /* head_score 超过 baseline + delta 进入 */
#define DMS_HEAD_RECOVER_DELTA     0.10f
#define DMS_EYE_CLOSED_MS          800
#define DMS_LONG_EYE_CLOSED_MS     1500
#define DMS_YAWN_MS                1000
#define DMS_HEAD_DOWN_MS           1500
#define DMS_FEATURE_EMA_ALPHA      0.65f   /* 越大越跟手，越小越平滑 */
#define DMS_EAR_SANE_MAX           0.60f   /* 单眼 EAR 物理上限，超过判关键点退化 */
#define DMS_CALIB_TIME_US          2000000ULL
