...
static const char *html_page =
    ...
    "<div style=\"margin-top:12px;color:#0f0;\">debug 叠加流：绿框 + 5点 + 106点</div>"
    "<img id=\"debug\" src=\"/stream_debug.mjpg\" alt=\"debug feed\">"
    ...
    "' ear:'+d.ear.toFixed(3)+' mar:'+d.mar.toFixed(2)+' head:'+d.head_down_score.toFixed(2)+"  
    "' 疲劳:'+(d.fatigue?'是':'否')+"
    "' lm106:'+(d.landmark_106?(d.landmark_106.found?'OK':'--'):'off');"
...
static void handle_status_json(int client_fd)
{
    ...
    off += snprintf(body + off, sizeof(body) - (size_t)off,
        ...
        "\"mar_threshold\":%.3f,"
        "\"head_baseline\":%.3f,"
        "\"landmark_106_found\":%d,"
        "\"feature_calibrated\":%d,"
        "\"feature_cost_ms\":%.3f",
        r->status,
        r->face_found,
        r->face_score,
        r->face_x, r->face_y, r->face_w, r->face_h,
        r->ear, r->mar, r->pitch, r->head_down_score,
        r->eye_closed, r->yawn, r->head_down, r->fatigue,
        r->left_ear, r->right_ear,
        r->ear_baseline, r->ear_threshold,
        r->mar_baseline, r->mar_threshold,
        r->head_baseline,
        r->landmark_106_found,
        r->feature_calibrated,
        r->feature_cost_us / 1000.0f);

#if DMS_ENABLE_LANDMARK_106
    const dms_face_landmark_106_result_t *lm = &r->landmark_106;
    off += snprintf(body + off, sizeof(body) - (size_t)off,
        ",\"landmark_106\":{\"found\":%d,\"total_ms\":%.3f,\"points\":[",
        lm->found, lm->total_us / 1000.0f);
    if (lm->found) {
        for (int i = 0; i < DMS_LANDMARK_106_NUM * 2; i++) {
            if ((size_t)off > sizeof(body) - 64) {
                break;
            }
            off += snprintf(body + off, sizeof(body) - (size_t)off,
                            "%s%.1f", (i == 0) ? "" : ",", lm->points[i]);
        }
    }
    off += snprintf(body + off, sizeof(body) - (size_t)off, "]}");
#else
    off += snprintf(body + off, sizeof(body) - (size_t)off,
        ",\"landmark_106\":{\"enabled\":0}");
#endif
